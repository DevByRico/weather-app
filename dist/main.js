...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // 🔁 ÅTERSTÄLL TILL LOKALT VÄDER om sökfältet rensas
    ...
    async function showForecast(city, countryCode) {
        const url = `https://api.openweathermap.org/data/2.5/forecast?q=${encodeURIComponent(city)},${countryCode}&appid=${apiKey}&units=metric&lang=${userLang}`;
        try {
            const response = await fetch(url);
            const data = await response.json();
